...
class ImageDataset3D(Dataset, Randomizable):
    """
    ...
    def __getitem__(self, index: int):
        ...
        new_transforms_list = deepcopy(self.transforms_list)
        if self.is_train:
            new_transforms_list.append(RandRotate90(prob=0.5, spatial_axes=(1, 2)))
            new_transforms_list.append(RandFlip(prob=0.5, spatial_axis=1))
            new_transforms_list.append(RandFlip(prob=0.5, spatial_axis=2))
        if self.is_rgb:
            new_transforms_list.append(RepeatChannel(repeats=3))
        self.transform = Compose(new_transforms_list)
